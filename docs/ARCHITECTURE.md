...
- Public signed events are pulled between explicitly configured mirrors through a durable content-addressed feed. Cursors are mirror-local; reset snapshots reconstruct current orders and terminal evidence. Exact replays are idempotent. A bounded causal queue retries a child that arrives before its signed prerequisite. Conflicting OrderV1 terms under one signer-bound id, two fills, or fill plus cancel are retained as equivocation evidence and suppressed. Unsigned compatibility rows, private orders, bearer tokens, maker presence, and IP metadata never federate.
...
- Live book: browsers aggregate each configured mirror's snapshot and SSE stream. Signed rows are verified locally and deduplicated by order id plus semantic digest; same-id digest conflicts are hidden and their id quarantine persists across browser reloads. Each HTTP or SSE payload is capped at 4 MiB and a snapshot at 200 rows, with one bad mirror isolated from the others. Unsigned rows are accepted only from the primary mirror. Every in-flight record persists its chosen `bookId`, so all later reads and mutations return to the same origin.
...
- **Federation boundary**: mirrors are untrusted relays. They verify every event, use canonical content hashes only for transport deduplication, and retain signed tombstones. Feed history, event size, pages, reset snapshots, peer response bodies, deferred dependencies, retained orders, intents, and conflict proofs are bounded. The dependency queue has both global and per-peer quotas; only an offending peer's source claims and cursor are reset. Peer failures and repeated reset churn back off independently. Private capabilities and unsigned liquidity stay origin-local. Partitioned maker equivocation remains possible and is handled as detectable evidence, not silently resolved by arrival order.
