...
- One order book process owns its data files at a time. An exclusive lease beside `orders.json` and its federation feed log is taken before either file is opened, so a second process on the same volume refuses to start. Two writers would interleave a whole-file orders rewrite with another process's feed appends and destroy both files. A crashed holder is recognised by its dead process id on this host, or by an expired heartbeat across PID namespaces. Ownership is proved again before every persisted write: a proven loss exits non-zero for the supervisor, and a lease that cannot be read refuses that one write and reports the fault in sanitized status. Operator detail is in [MIRROR_OPERATORS.md](MIRROR_OPERATORS.md).
