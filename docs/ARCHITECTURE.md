...
- v1 order book (implemented, `server/`) is a dedicated node service; it is **coordination only**, never custody. Interactive listings carry portable ML-DSA-87 OrderV1 maker proofs (MyQRLWallet native typed data or official-wallet EIP-712 v4), verified by both server and taker browser. It carries order parameters, the taker's addresses and the maker's hashlock announcement; clients still re-verify recipients, amounts and timeouts against on-chain HTLC state before committing funds or revealing the secret, so a malicious order book can waste time but cannot redirect a swap. Losing it degrades the protocol to out-of-band coordination, it does not strand funds. Mirror synchronization and signed cancellation tombstones remain separate federation work.
...
- Taker release: accepting mints a taker bearer token (mirroring the maker's), and the taker can walk away via an authorized release. Before the maker locks, the order returns to the book untouched; after, the listing stays `locking` (chain state governs the funds) but the take stops counting against the taker's per-IP concurrency slots. The daily take cap still counts locking-phase releases, since those already cost the maker gas and a lockup. Clients fire the release on walk-away and on discard; it is book-keeping only, so a lost release costs nothing but a slot until the sweep.
