...
Status: September 2026 private v3 release. Fresh target-bound HTLCs are deployed on Sepolia and private QRL v3. The portable V2 order book, frontend flow, and self-hosted LP kit share one pinned deployment profile. Solver mode (Phase 3) is planned.

## 1. Goal and constraints

Swap Ethereum-leg assets against native QRL on the private v3 QRVM network with atomic settlement and no custodian.

Constraints that shape the design:

- **Private QRL v3 uses chain ID 3151909 and 64-byte addresses.** Clients pin its genesis in `config/protocol-v2.json` and verify the live chain identity before QRL operations. Sepolia retains chain ID 11155111 and 20-byte addresses. The private testnet release establishes no mainnet readiness guarantee.
- **One reviewed source, two explicit VM targets.** Contract sources are Hyperion-only (`.hyp`, compiled with native `hypc`). The EVM-256 artifact for Ethereum and QRVM-512 artifact for QRL share the same source bundle and ABI, while their target-specific bytecode differs. Manifests bind each artifact to its target, compiler version, address width, source hash, and codegen settings. The canonical gate runs the EVM artifact on a throwaway anvil and exercises full-width Q128 behavior on QRVM; there is no Solidity mirror and no Foundry suite.
...
One source, `contracts/hyperion/HTLC.hyp`, compiled into explicit EVM and QRL artifacts with ABI parity enforced between them. Current private v3 addresses, build provenance, and verified runtime hashes are recorded in `docs/DEPLOYMENTS.md`. Network v3 is distinct from the HTLCv2 open-recipient contract interface and portable signing wire V2.

A second contract, `contracts/hyperion/HTLCv3.hyp`, lives in the same reviewed bundle and is **not deployed anywhere**. It is the payout redesign required by issue #47: a settlement writes its terminal state first and attempts delivery second, and a delivery that cannot go through becomes a credit the payee owns and withdraws to a destination it names. The claim stands, with its preimage already public. HTLCv2 stays live on testnet with its own addresses and is unchanged, byte for byte. HTLCv3 ships as a fresh immutable deployment once an independent audit and the finality policy in [FINALITY.md](FINALITY.md) are both signed off; the audit package is [audit/HTLCV3_SCOPE.md](audit/HTLCV3_SCOPE.md). Client integration is a separate change that has not been written yet.
...
- `lockNative(hashlock, recipient, timeout)` / `lockToken(hashlock, recipient, token, amount, timeout)` pull funds in (`msg.value` for the native coin, ERC-20 `transferFrom` for tokens) and store the record keyed directly by `hashlock`; the hashlock is the swap id on both chains. For ERC-20 locks the contract verifies its balance grew by exactly `amount` and rejects the lock otherwise (`UnsupportedToken`): a fee-on-transfer or rebasing token could otherwise record a swap whose later payout would be funded by other swaps' escrow.
- `claim(hashlock, preimage)` verifies `sha256(secret) == hashlock`, pays `recipient`, stores the revealed secret in the record, emits it in the event. **Permissionless**: anyone may call it; the payout target is fixed at lock time, or, for open-recipient locks (below), at the initiator's one-time `assign()`, and `claim` reverts while it is unset. Either way a sponsored claim (section 5) can only pay the target the fund-owner chose.
- `refund(hashlock)` after `timeout`, pays `initiator`.
...
Rationale: the secret is revealed on the responder's leg at claim time. The responder then needs the remaining `T1 - now` to claim the initiator's leg. If `T1` were short, a claim near `T2` would leave the responder unable to collect. Current testnet defaults are T2 = 1h and T1 = 2h, with a 48h initiator timeout for pre-funded listings. These are private-testnet parameters; a real-value deployment requires independently reviewed finality margins on both chains.
...
- The order book (`server/`) is **coordination only**, never custody. New listings are portable ML-DSA-87 OrderV2 objects. Takers propose a short-lived signed FillIntentV2; the maker signs exactly one terminal FillV2, which selects the taker and announces the hashlock and timeouts, or CancelV2. A release-secret commitment lets the taker withdraw without another wallet prompt, but a release never reopens OrderV2. Clients verify the proof chain and still re-verify recipients, amounts, token addresses, and timeouts against on-chain HTLC state before committing funds or revealing a secret.
- OrderV2 also signs origin capability commitments. The maker creates a raw 32-byte maker token and, for a private order, a raw 32-byte share token before wallet authorization. Separate NUL-terminated SHA-256 domains commit those raw bytes. Public orders sign a zero share commitment; private orders sign a nonzero commitment. The client stages the exact signed request and raw preimages before its first POST, then retries that same envelope until its origin authenticates the response. Raw tokens remain local and never enter public proofs, federation events, or application logs.
- Public signed events are pulled between at most 16 explicitly configured mirrors through a durable content-addressed feed. Cursors are mirror-local and intentionally in memory; a restart requests a reset snapshot that reconstructs current orders and terminal evidence. Exact replays are idempotent. A two-worker pool gives each peer sync one total deadline, validates cursor continuity and duplicate ids, and checkpoints completed incremental pages. A bounded causal queue retries a child that arrives before its signed prerequisite. Conflicting OrderV2 terms under one signer-bound id, two fills, or fill plus cancel are retained as equivocation evidence and suppressed. Unsigned compatibility rows, private orders, bearer capabilities, maker presence, and IP metadata never federate. Sanitized status exposes public peer labels, state, counters, and timestamps without URLs, cursors, feed ids, proofs, accounts, capabilities, or raw errors.
- Operators may add the optional `server/compose.tor.yaml` overlay. It builds a digest-pinned Debian C Tor sidecar locally, publishes a v3 onion service on virtual port 80 to `orderbook:8091`, and gives the order book a remote-DNS SOCKS route at `socks5h://tor:9050` for onion peers. The overlay enables onion-only peer validation, replaces the order book's network list with the internal `tor-socks` network, removes its published port, and uses a separate hardened proxy to preserve the `127.0.0.1:8091` administration endpoint. Startup rejects non-onion peers, and the order book has no direct clearnet egress. Every configured peer in this topology is a public-lane onion position with a `-` token sentinel. Separate persistent volumes hold the onion identity and Tor guard/client state.
...
- Portable acceptance flow: a taker signs a FillIntentV2 against the exact OrderV2 digest. The maker selects first-come: priority is the signed issuance clamped to arrival at the maker's book (`max(issuedAt, receivedAt)`), then issuance, then the semantic intent digest. The taker chooses `issuedAt`, so backdating buys no priority over an earlier arrival; a book that under-reports `receivedAt` can only fall back to issuance order. Proposals relayed from another mirror count as received at pull time, trailing local submissions by up to one pull interval. The maker persists its secret and exact terminal proof before publish, then durably records an authenticated FillV2 acknowledgment before its first lock. The taker funds only after independently verifying FillV2, its response deadline, and the initiator escrow at the configured confirmation depth.
- Portable release: the taker commits a random release secret in FillIntentV2. Revealing it marks the intent or selected fill released and tells the maker to stop. The reference LP persists an authenticated release observation as sticky. It never starts or repeats a lock afterward. If a prior lock attempt may have landed, the LP retains recovery state and continues chain-authorized claim or refund handling. The consumed OrderV2 remains terminal; continued quoting requires a fresh signed order. Legacy origin-local rows retain bearer-token release and take-by-terms only for compatibility.
...
- **QRL side**: `@qrlwallet/connect` 5.0.1 or MyQRLWallet Extension. Relay pairing uses `qrlconnect://` QR/deep link and an encrypted session with explicit wallet consent. Portable V2 uses `qrl_signMessage`, a fixed ordered JSON encoding, and SDK signer-bound ML-DSA-87 verification. The domain includes both chain IDs, both HTLCs, and exact QRL genesis. Typed-data-only providers are unsupported for portable V2. QRL contract calls use 64-byte QRVM ABI slots; Ethereum continues to use 32-byte EVM slots. Every QRL transaction includes the pinned chain ID, which the relay SDK checks before queuing and again before transport.
...
- **Sponsored claims**: because `claim` is permissionless with a fixed recipient, a swapper receiving QRL does not need QRL gas. In protocol mode the reference market maker sponsors the taker's final claim on its own lock by default (`MM_SPONSOR_CLAIMS`), in both directions, as soon as its own reveal is at confirmation depth. The claim can be submitted by the solver or a relayer once the secret is disclosed to it; funds can only go to the recipient fixed at lock time. Disclosure order is safe: by the time the user discloses the secret, the QRL leg is locked in their favor, and the WETH leg is what the solver is owed anyway. This replaces the "generated passkey wallet + faucet funding" approach from the community prototype with plain self-custody.
...
- **Portable order single use**: OrderV2, FillIntentV2, FillV2, and CancelV2 bind both accounts, deployment, nonces, deadlines, and canonical semantic digests. Order ids bind the maker QRL account and nonce under the `QuantaSwap OrderV2 id` SHA-256 domain. OrderV2 additionally binds domain-separated maker and share capability commitments. A maker's FillV2 or CancelV2 is terminal. The sole proof scheme is `qrl-sign-message-v2`, with full signer identity derived from the descriptor and ML-DSA-87 public key. Contradictory signed children quarantine the order. ReleaseV2 reveals only a committed cancellation capability and cannot authorize funds or reopen a listing.
- **Loss-safe signed create**: browser and headless makers persist the complete create envelope, including raw capability preimages, before transport. An exact retry returns the same authenticated order and echoes only the preimages submitted again by that client. The mirror stores commitments rather than signed-create preimages. A lost local stage is unrecoverable. Earlier deployment and V1 proof files are preserved for recovery under their original profile; the v3 release does not migrate them into active V2 state. Browser swap, order, staging, and quarantine keys include the current deployment fingerprint.
...
- **Tor transport boundary**: the optional sidecar protects onion-service transport and routes configured onion federation pulls through Tor. It does not route clearnet federation peers, browser wallet providers, market-maker RPC calls, HTLC transactions, or chain-indexer traffic. Ethereum and QRL addresses, amounts, contract calls, transaction hashes, and timing remain public on their respective chains. RPC operators and wallet providers can observe their own requests. Onion access therefore improves mirror-network privacy without creating on-chain anonymity.
- **Onion source collapse**: direct onion requests arrive at the order book from one internal Tor-sidecar address. All onion users share per-source request, feed-reset, SSE, and concurrency quotas, so one client can consume the shared allowance. The sidecar provides no authenticated client-address header. Proxy trust remains disabled, and operators monitor aggregate limits and `429` responses.
...
- **Reference LP response authentication**: the headless maker accepts create, fill, cancel, and later order views only when the response reproduces its exact signed OrderV2 and terminal artifacts, semantic digests, terms, and expected state without conflict evidence. A malformed or contradictory 2xx response leaves recovery state intact and blocks funding. A first lock requires a persisted authenticated FillV2 acknowledgment. Authenticated release observation is sticky. During a book outage, chain actions continue only after that acknowledgment, a prior lock attempt, or observed HTLC exposure. A prior lock attempt is never automatically abandoned while inclusion remains ambiguous.
...
- **Payout delivery cannot roll back a claim (HTLCv3, not yet deployed)**: HTLCv2 pushes the payout inside `claim`, so a payout that reverts rolls the claim back to Open while the preimage may already be public through calldata, the RPC or the mempool, and the counterparty can then claim the other leg. HTLCv3 splits the two: the terminal state and the preimage are written unconditionally, delivery is attempted afterwards in a gas-bounded child frame, and a delivery that fails becomes a credit owned by the payee. Value is conserved per token, a failed delivery moves nothing, and only the credited account can move its credit. The same fallback covers `refund` and `release`, which also removes HTLCv2's stranded-refund case for a blocklisted initiator. Integrations must submit settlements with `estimateGas + DELIVERY_GAS_LIMIT + DELIVERY_GAS_RESERVE`, published on-chain through `deliveryGasPolicy()`, because a bare gas estimate lands on the cheaper credit path and defers a payout that would have gone through.
- **No admin keys in the HTLC contracts**: no pause, no upgrade, no owner. What is deployed is final; fixes ship as new deployments.

## 8. Deployment

- **Domain**: quantaswap.io. Deployment operations and private infrastructure configuration are documented outside the public repository.
- **Testnets**: Sepolia (11155111) and private QRL v3 (3151909). QRL RPC: `https://qrlwallet.com/api/qrl-rpc/testnet`. The `qrl_*` namespace uses uppercase Q-prefixed 64-byte addresses. Shared configuration lives in `config/protocol-v2.json`; server and market-maker packages must retain this sibling config directory at runtime.
- **Toolchain**: `npm run compile` builds `build/hyperion/evm/` with the configured EVM-compatible compiler and `build/hyperion/qrl/` with the QRL Q128 compiler. `config/hyperion-targets.json` records the required compiler versions, address widths, VM formats, optimizer settings, and `viaIR: true`; `EVM_HYPC_BIN` and `QRL_HYPC_BIN` may select matching local binaries. Build parity requires one source-bundle hash and identical ABIs. Artifact loaders reject target, compiler, codegen, source, or content mismatches before deployment or smoke traffic. `npm test` runs manifest guards, the EVM artifact on a throwaway anvil, and full-width Q128 semantic tests on QRVM. Deploy commands remain `npm run deploy:qrl` and `npm run deploy:eth`; live smokes use `scripts/smoke-{qrl,eth}.js`.
- **Optional onion transport**: `server/compose.tor.yaml` adds the hardened C Tor sidecar and persistent v3 identity. Operator startup, probe, encrypted identity backup, and recovery procedures are in [MIRROR_OPERATORS.md](MIRROR_OPERATORS.md).
...
6. Mainnet network availability and independent security qualification (external; gates Phase 4).
