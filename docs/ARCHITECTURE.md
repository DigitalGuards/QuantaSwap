...
- **QRL side**: `@qrlwallet/connect` 5.0.0 or MyQRLWallet Extension. Relay pairing uses `qrlconnect://` QR/deep link and an encrypted session with explicit wallet consent. Portable V2 uses `qrl_signMessage`, a fixed ordered JSON encoding, and SDK signer-bound ML-DSA-87 verification. The domain includes both chain IDs, both HTLCs, and exact QRL genesis. Typed-data-only providers are unsupported for portable V2. QRL contract calls use 64-byte QRVM ABI slots; Ethereum continues to use 32-byte EVM slots.
