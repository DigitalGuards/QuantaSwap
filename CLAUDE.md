...
- Order book: `server/`, plain node:http + hardened TS, zero runtime deps, coordination only (clients re-verify everything on-chain). Per-IP take caps (4 concurrent, 24/day) keep one visitor from draining the book. Accepting mints a taker token; `POST /orders/:id/release` is the taker's walk-away (pre-lock: order reopens; post-lock: frees the concurrency slot only). `POST /orders/take` fills the best open order by bounds (race/staleness-proof takes); makers heartbeat (`POST /orders/:id/heartbeat`, read-class rate limit, in-memory presence, 90s TTL) and offline-maker orders are skipped by take-by-terms; `GET /orders/stream` is the SSE book push (clients poll only as fallback). Gate: `npm test` in `server/` (build + lifecycle smoke). Prod: pm2 `quantaswap-orderbook` on 127.0.0.1:8091 behind the vhost's `/api` proxy.
